...
# data = soup.select_one("#__next > div > main")

# ----------------------------------------------------------------------------------------------

# for i in range(5):
#   # 파일 불러와서 저장하기 - 1회
#   url = f"http://www.yeogi.com/domestic-accommodations?keyword=%EA%B0%95%EB%A6%89&checkIn=2024-10-23&checkOut=2024-10-24&personal=2&freeForm=false&page={i+1}"
...
# 평가수 500 이상, 평점 9.0 이상 금액 120000 이하만 출력하시오.
...
#제목, 평점, 평가수, 가격, 이미지, 링크주소
print("[ 강릉 숙소 ]")

# 20 출력 - 평가수 500명 이하 제외
for idx,sell in enumerate(sells):
  try:
    num = sell.select_one("span.css-oj6onp").text.strip()[:-4]
    num = int(num.replace(",",""))
    rating = float(sell.select_one("div.css-1ar2n2o > div > span").text.strip())
    ...
    if num < 500 or rating < 9.0 or price > 120000:
      ...
      continue
    print(f'{(i*20)+idx+1}.')
    print("상품명 : ",sell.select_one("div.css-8fn780 > h3").text.strip())
    print("평점 : ",rating)
    print("평가수 : ",num)
    print("금액 : ",price)
    # img = sell.select_one("div.css-nl3cnv")
    # print("이미지 : ",img)
    link = sell['href']
