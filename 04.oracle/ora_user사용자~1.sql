...
-- select * (모든 컬럼을 의미) from 테이블명
select *from employees;

select employee_id,emp_name,salary from employees;
select EMP_NAME from EMPLOYEES;
-- 명령어는 대소문자를 구별하지 않지만 값들은 대소문자를 따진다.
...
-- 사원번호, 사원명, 직급번호(job_id) SH CLERK 사워만 출력하시오.
...
-- 데이터추가(insert), 삭제(delete), 검색(select), 수정(update)
...
select * from employees;
-- 사원월급이 달러에서 원화로 환산 하시오.
select emp_name,salary,to_char(salary*1384.66,'999,999,999') from employees;
