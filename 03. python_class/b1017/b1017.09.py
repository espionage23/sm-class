  ...
  if choice == "1":
    ...
    for m in members:
      if input_id == m['id'] and input_pw == m['pw']:
        session_info = m
        print(f"{session_info['name']}님 웰컴")
        # 회원정보
        while True:
          print("           [ SM-SHOP ]")
    ...
    for m in members:
      if new_id == m['id']:
        print("아이디 혹은 비밀번호를 사용한 유저가 있습니다. 다시 시도하라")
        flag = 1
        break
    if flag == 1:
      continue
      ...
      new_pw = input("사용할 비번 >>")
      name = input("이름이 뭐고 >>")
      nicName = input("닉네임 뭐고 >>")
      address = input("주소가 뭐고 >>")
      money = input("센터에 얼마 있는가 >>")
      new = [new_id,new_pw,name,nicName,address,money]
      members.append(dict(zip(m_title,new)))
      print(members)
