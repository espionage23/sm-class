-- join
-- inner join - equi join, non-equi join, self sjoin,outer join

-- employees, 사원번호, 사원명, 부서번호, 부서명, 출력하시오.
...
-- 두 테이블간 공통분모의 컬럼 없이 데이터를 가져오는 방법 : non-equi join
...
-- avg를 가지고 stu_grade 활용해서 result를 입력하시오.
update stu a set result = (
select results from (select no,grade as results from stu,stu_grade
...
select nvl(manager_id,'ceo') from employees;  -- 에러 : 타입이 다르기 때문에
select nvl(to_char(manager_id),'ceo') manager_id from employees;

-- self join manger_id, 매니저의 이름을 출력하시오.
-- self join시 106개가 출력
...
-- 조건 1: 위쪽 쿼리문과 아래쪽 쿼리문 개수가 동일해야한다.
-- 조건 2: 타입이 무조건 일치해야한다.
...
-- employees dpartment_id가 50인 사원 검색 -> 부서번호, 부서이름
select a.department_id,department_name
from employees a, departments b
where a.department_id = b.department_id and a.department_id = 50;
-- employees salary 5000 and 8000 작은 사원의 부서번호, 부서이름
select a.department_id,department_name, salary
from employees a, departments b
where a.department_id = b.department_id and salary between 5000 and 8000;
...
-- row_number
select * from(
select row_number() over(order by avg desc) rnum, rank() over(order by avg desc), a.* from students a)
where rnum between 11 and 20;
...
-- rank() 값 rank 컬럼에 모두 입력
update students a set rank = (
select ranks from
(select no,name,avg,rank, rank() over(order by avg desc) ranks from students)b
where a.no = b.no 
);
...
-- 사원번호, 사원명, 이메일, 전화번호, 입사일, 부서번호, 부서명
...
select employee_id as e_id, emp_name as e_mane from employees_view;
...
-- 복합 view: 2개 이상의 테이블로 조인, 함수사용, group by 같은 경우는 insert,  update, delete 불가
...
-- students 테이블 no가 가장 큰수는?
select max(no) from students;

-- avg 소수점 2자리까지만 출력.
select no,name,kor,eng,math,total,round(avg,2),rank,substr(sdate,1,4)
from students;
